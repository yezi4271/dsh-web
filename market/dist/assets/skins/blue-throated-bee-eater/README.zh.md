...
- **纯资产**：`skin.json`（v2 清单）+ `skin.css`（token 重映射）+
  ...
  `assets/selected-grass.svg`（选中行草地边）+ `assets/selected-bee-scene.svg`
  （花与蜂角落场景）+ 双语 README 与 NOTICE。无 package.json、无构建步骤；
  皮肤中心包是唯一加载器。
- **选中元素草地**：选中的侧栏行与标签页底缘是一片低矮草地；选中行左边框
  附近从草地长出一朵花，一只大写意墨蜂头朝花俯冲而来、即将落上花心——
  浓墨阔笔、枯笔飞白、赭色晕染腹与雾气化开的翅膀——正是蜂虎的“餐桌”。场景画在标题左侧的空置 marker 槽位区域，锚定 shell
